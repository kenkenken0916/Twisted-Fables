    ...
    switch (gameState->status) {
        case CHOOSE_IDENTITY:
            if (choice < 1 || choice > 3) {
                ERROR_LOG("Invalid identity choice: %d", choice);
                printf("Please choose a valid identity (1-3)\n");
                return true;
            }
            handle_choose_identity(gameState, choice);
            break;
...
void handle_choose_identity(game* gameState, int8_t choice) {
    DEBUG_LOG("Handling identity choice: Player=%d, Choice=%d", 
             gameState->now_turn_player_id, choice);
             
    player* current_player = &gameState->players[gameState->now_turn_player_id];
    current_player->alice.identity = choice;
    
    INFO_LOG("Player %d chose identity %d", 
